...
public class ElasticsearchTest {
  ...
  public static void startElasticsearchCreateLocalClient() throws Exception {
    ...
    container.copyFileFromContainer("/usr/share/elasticsearch/config/certs/http_ca.crt",
        "D:/bootstrap/docker-elk/demoelastic/pogChamp/http_ca.crt");
    Path path = Paths.get("D:/bootstrap/docker-elk/demoelastic/pogChamp/http_ca.crt");
    byte[] certAsBytes = Files.readAllBytes(path);

    HttpHost host = new HttpHost("localhost", container.getMappedPort(9200), "https");
    final CredentialsProvider credentialsProvider = new BasicCredentialsProvider();
    credentialsProvider
        .setCredentials(AuthScope.ANY, new UsernamePasswordCredentials("elastic", "s3cret"));
...
//    }
  ...
  public void testPagination() throws Exception {
    ...
    assertThat(page.get()).hasSize(10);
    List<String> firstPageIds = page.get().stream().map(Product::getId)
        .collect(Collectors.toList());
    List<String> secondPageIds = secondPage.get().stream().map(Product::getId)
        .collect(Collectors.toList());
  ...
  public void testSearchAfter() throws Exception {
    ...
    final SearchResponse<Object> response = client.search(b -> b
            .index(INDEX)
            .query(
                qb -> qb.match(mqb -> mqb.field("name").query(builder -> builder.stringValue("Name"))))
            .sort(sb -> sb.field(FieldSort.of(fs -> fs.field("price").order(SortOrder.Desc))))
        , Object.class);

    final List<String> ids = response.hits().hits().stream().map(Hit::id)
        .collect(Collectors.toList());
    final List<String> sort = response.hits().hits().get(response.hits().hits().size() - 1).sort();
    final JsonObject sortOrder = Json.createObjectBuilder(
        Collections.singletonMap("price", Collections.singletonMap("order", "desc"))).build();
    ...
    final List<String> searchAfterIds = searchAfterResponse.hits().hits().stream().map(Hit::id)
        .collect(Collectors.toList());
  ...
  public void testQueryBuilders() throws Exception {
    ...
    final SearchResponse<Object> response = client
        .search(b -> b.index(INDEX).query(q -> q.bool(builder ->
            builder
